...
def save_images(img_list):
    ...
    for idx, img in enumerate(img_list):
        ...
        # img_id = img['id']
        img_id = f"{img['latitude']}_{img['longitude']}"
        try:
            img_data = requests.get(img_url).content
            with open(f'/home/mgfos207/Desktop/Springboard/Module 28/assets/data/airbnb_images/{img_id}.jpg', 'wb') as handler:
...
def get_img_data(df):
    select_features = ['picture_url', 'latitude', 'longitude']
...
def main():
    ...
    df = pd.read_csv("/home/mgfos207/Desktop/Springboard/Module 28/assets/data/modified/preprocessing_final.csv")
    # df = df.loc[df['50_percent_or_more_booking'] == 1]
